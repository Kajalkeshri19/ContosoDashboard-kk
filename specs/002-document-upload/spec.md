...
3. **Given** a multi-file upload contains both valid and invalid files, **When** the upload is submitted, **Then** each result is reported clearly and no rejected file becomes accessible.
...
1. **Given** a user owns a document, **When** they share it with a permitted user or team, **Then** the recipient receives an in-app notification and the document appears in their shared-with-me view.
2. **Given** a user is not the owner or an authorized project manager, **When** they attempt to edit, replace, delete, or share a document, **Then** the action is denied and the document remains unchanged.
...
- **FR-007**: The system MUST ensure that a failed upload cannot leave an accessible orphan file or incomplete document record.
...
- **FR-012**: The system MUST enforce document access in the service or resource boundary, including direct download and preview requests, and MUST respect the existing employee, team lead, project manager, and administrator roles.
...
- **FR-015**: The system MUST allow owners to share documents with specific users or teams and MUST notify recipients in the application.
...
- **FR-020**: The system MUST record upload, download, preview, replacement, deletion, and sharing events with the actor, document, timestamp, and outcome for administrator audit reporting.
...
- **FR-022**: The training implementation MUST clearly identify malware scanning as deployment-dependent; where no offline scanner is available, uploads MUST be limited to the approved file policy and MUST NOT claim production malware protection.

### Key Entities

- **Document**: A work file and its searchable metadata, ownership, project association, storage reference, and lifecycle state.
- **DocumentShare**: A permission grant connecting a document to a specific user or team, including grantor, recipient, and grant status.
...
- Malware scanning requires an available deployment service. The offline training environment does not promise a full antivirus scan, so the release must document this limitation.
